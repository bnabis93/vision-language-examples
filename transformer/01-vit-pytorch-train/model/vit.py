...
class FeedForward(nn.Module):
    def __init__(self, dim, mlp_dim, dropout: float = 0.0):
...
class Transformer(nn.Module):
    def __init__(self, dim, depth, heads, dim_head, mlp_dim, dropout: float = 0.0):
        ...
        for _ in range(depth):
            # Self-attention -> LayerNorm -> MLP -> LayerNorm
            self.layers.append(
                nn.ModuleList(
                    [
                        PreNorm(
...
class ViT(nn.Module):
    def __init__(
        ...
        *,
        ...
        # Rearrange: (b, c, h, w) -> (b, h*w, c), where c = patch_height * patch_width * channels
    ...
    def forward(self, img):
        ...
        # shape of x: (b, n, d), b = batch size, n = number of patches, d = dimension of each patch
        b, n, _ = x.shape
